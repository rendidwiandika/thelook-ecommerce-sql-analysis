...
monthly_leakage_audit AS (
    SELECT 
        ...
        -- fulfillment health: average time from order to shipment, and shipment to delivery
        -- (only computed for orders that actually reached that stage)
        ROUND(AVG(CASE WHEN status IN ('complete', 'shipped') THEN handling_days END), 1) AS avg_handling_days,
        ROUND(AVG(CASE WHEN status IN ('complete', 'shipped') THEN shipping_days END), 1) AS avg_shipping_days
